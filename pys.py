...
def listar_pys_por_tipo(pys_list, tipo):
    for pys in pys_list:
        if pys['tipo'].lower() == tipo.lower():
            print(f"Título: {pys['titulo']}, Productora: {pys['productora']}, Géneros: {', '.join(pys['generos'])}, Fecha de estreno: {pys['fecha_estreno']}, Valoración de usuarios: {pys['valoracion']}")
    else:
        print(f"No se encontraron {tipo}s en la lista.")

def listar_pys_por_valoracion(pys_list, valoracion):
    for pys in pys_list:
        if pys['valoracion'] >= valoracion:
            print(f"Título: {pys['titulo']}, Productora: {pys['productora']}, Géneros: {', '.join(pys['generos'])}, Tipo: {pys['tipo']}, Fecha de estreno: {pys['fecha_estreno']}, Valoración de usuarios: {pys['valoracion']}")
    else:
        print(f"No se encontraron PyS con valoración mayor o igual a {valoracion} en la lista.")

def listar_pys_por_genero(pys_list, genero):
    for pys in pys_list:
        if genero.lower() in [g.lower() for g in pys['generos']]:
            print(f"Título: {pys['titulo']}, Productora: {pys['productora']}, Tipo: {pys['tipo']}, Fecha de estreno: {pys['fecha_estreno']}, Valoración de usuarios: {pys['valoracion']}")
    else:
        print(f"No se encontraron PyS con el género '{genero}' en la lista."),
...
while True:
    ...
    print("6. Listar series")
    print("7. Listar películas o series por valoración")
    print("8. Listar películas o series por género")
    print("10. Salir")
    ...
    if opcion == 1:
        ...
        listar_pys_por_tipo(peliculas_y_series, "Película")
    elif opcion == 6:
        listar_pys_por_tipo(peliculas_y_series, "Serie")
    elif opcion == 7:
        valoracion = float(input("Ingrese el valor de valoración mínimo: "))
        listar_pys_por_valoracion(peliculas_y_series, valoracion)
    elif opcion == 8:
        genero = input("Ingrese el género a buscar: ")
        listar_pys_por_genero(peliculas_y_series, genero)
    elif opcion == 10:
        print("¡Hasta luego!")
        break
    else:
